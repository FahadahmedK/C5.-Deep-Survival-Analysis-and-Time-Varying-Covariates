...
Original file is located at
    https://colab.research.google.com/github/pryplotsky/C5.-Deep-Survival-Analysis-and-Time-Varying-Covariates/blob/main/CPH001.ipynb
...
ctv.fit(temp.X_train, id_col="id", event_col="status_time", start_col="start", stop_col="time", show_progress=True)
ctv.print_summary()
ctv.plot()

ctv.predict_partial_hazard(temp.X_test)
...
survival.fit(temp.df['time'], temp.df['default_time'])
survival.plot()
...
concordance_index(temp.X_test['time'], ctv.predict_partial_hazard(temp.X_test))

"""# Survival Curve for a couple individuals (seems not to work)"""

#ctv.plot_partial_effects_on_outcome(covariates='id', values=[ 4, 6, 8, 10], cmap='coolwarm')
ctv.plot_partial_effects_on_outcome('id', [ 4, 6, 8, 10], cmap='coolwarm')
